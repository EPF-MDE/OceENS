...
def seed_programs(session: Session):
    """Remplit la table programs depuis seed_data/Program_list.csv."""

    # Clean table first
    session.exec(delete(Program))

    csv_path = DATA_DIR / "Program_list.csv"

    if not csv_path.exists():
        logger.warning("[SEED] %s introuvable.", csv_path)
        return
...
def seed_answers(session: Session):
    ...
    for survey_data in SEEDED_SURVEYS:
        csv_path = DATA_DIR / survey_data["answers_file"]
