...
#
# Le code vit sous src/oceens/ : la racine du dépôt est trois niveaux au-dessus
# de ce fichier, et on la reconnaît à son pyproject.toml. Installé ailleurs que
# depuis une copie de travail (site-packages), ce repère n'existe pas et la
# base retombe sur le répertoire courant — le seul endroit que l'appelant
# choisit. Pour fixer l'emplacement dans tous les cas : LOCAL_DATABASE_DIR.
_CHECKOUT_ROOT = Path(__file__).resolve().parents[3]
PROJECT_ROOT = (
    _CHECKOUT_ROOT if (_CHECKOUT_ROOT / "pyproject.toml").exists() else Path.cwd()
)
